...
if False:
    config = dict(
        dirpath=PATH + "/../../data/cmip6/historical/CESM2/sst/",
        prefix = "b.e21.BHISTcmip6.f09_g17.LE2-1001.001.pop.h.",
        var='SST',
        n_cpus = 8,
        outpath=PATH + "/../../data/processed_data/test/"
    )
else:
...
orig_ds = xr.open_mfdataset(os.path.join(config['dirpath'], config['prefix']) + "*.nc", 
                            chunks={'time': 240},
                            combine='by_coords')
orig_da = orig_ds[var] #.isel(time=slice(0,480)) # TODO: For testing
