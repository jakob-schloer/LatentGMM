...
outfile = "../../data/reanalysis/monthly/ssta_merged_dataset.nc"
...
# %%
# Sample data to get 4 datapoints per month
sampled_indices = []
unique_dates = np.arange('1901-01', '2023-01-01', dtype='datetime64[M]')
for date in unique_dates:
    indices = np.where(data['time'] == date)[0]
    sampled_indices.append(np.random.choice(indices, size=4, replace=False))

sampled_indices = np.array(sampled_indices).flatten()

data_sampled = data.isel(time=sampled_indices)
